...
export function BusinessPage() {
  ...
  function exportBuyList() {
    ...
    downloadText(
      ...
      ["product,category,supplier,qty,est_price,margin_pct,traffic_potential,priority,competitor", ...rows].join("\n"),
    );
          ...
          <div className="flex flex-wrap items-start gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-indigo-900">
                Your first baseline scan is pending
              </p>
              <p className="mt-0.5 text-xs text-indigo-800">
                SEO, GEO, traffic, review and social numbers appear here after the first scan of{" "}
                {profile.primaryDomain || "your domain"} completes. Suppliers, competitors and clients
                are already set up from onboarding.
              </p>
            </div>
          </div>
        </Card>
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <Card>
          ...
          <div className="grid gap-4 px-4 py-4 sm:grid-cols-2">
            <div className="rounded-xl bg-slate-50 p-3">
              ...
              <p className="mt-2 text-[11px] text-slate-600">
                ...
                {topSeoKeywords.filter((k) => k.position <= 10).length} of{" "}
          ...
          <div className="border-t border-slate-100 px-4 py-3">
            ...
            <AreaChart data={geoVisibility} color="#0d9488" valueFormat={(n) => `${n}/100`} height={110} />
          </div>
        </Card>
          ...
          <div className="grid gap-5 px-4 py-4 md:grid-cols-2">
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3 text-[11px] text-slate-600">
                ...
                <span>·</span>
                <span>
                  {Math.round(myBusiness.monthlyVisits * (myBusiness.conversionRate / 100)).toLocaleString()}{" "}
                  orders last month
                </span>
              </div>
            </div>
                ...
                {topSeoKeywords.map((k) => (
                  <tr key={k.keyword} className="hover:bg-slate-50/70">
                    ...
                    <Td className="text-center">
                      <Badge tone={k.position <= 3 ? "good" : k.position <= 10 ? "info" : "warn"}>
                        #{k.position}
                      </Badge>
                    </Td>
              ...
              <tbody className="divide-y divide-slate-100">
                {topGeoKeywords.map((k) => (
                  <tr key={k.prompt} className="hover:bg-slate-50/70">
                    ...
                    <Td className="text-center">
                      <Badge tone={k.position <= 3 ? "good" : k.position <= 8 ? "info" : "warn"}>
                        #{k.position}
                      </Badge>
                    </Td>
                    <Td className="text-right">
      ...
      <div className="grid gap-5 xl:grid-cols-3">
              ...
              <div key={s.source} className="grid gap-3 px-4 py-4 md:grid-cols-3">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-900">{s.score.toFixed(1)}</span>
        ...
        <Card>
          <CardHead
            ...
            subtitle={`Scanned ${relativeTime(lastScan)} · next scan ${shortDate(latestReviewScan.nextScanAt)}`}
            action={
                  ...
                  <Td>
                    <span className="flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={() =>
                          downloadAdPack(a.product, a.sku, a.downloadUrl ?? a.storagePath ?? a.figmaUrl, a.sizeMb)
                        }
                      >
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <Card className="xl:col-span-2">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {socialScores.map((s) => (
                  <tr key={s.platform} className="hover:bg-slate-50/70">
                    ...
                    <Td className="text-right text-xs text-slate-600">{compact(s.followers)}</Td>
                    ...
                    <Td>
                      <Badge
                        tone={s.focus === "high" ? "bad" : s.focus === "medium" ? "warn" : "neutral"}
                      >
                        {s.focus} priority
                      </Badge>
                      <span className="mt-1 block max-w-64 text-[11px] text-slate-500">{s.reason}</span>
                    </Td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="border-t border-slate-100 px-4 py-3 text-[11px] text-slate-600">
            ...
            — those are the two channels where competitors pull the most extra traffic and where your
            ...
            {competitors.map((c) => (
              <li key={c.id} className="px-4 py-3">
                ...
                <div className="mt-1.5 flex flex-wrap gap-3 text-[11px] text-slate-500">
                  ...
                  <span>{c.social[0]?.followers.toLocaleString()} on {c.social[0]?.platform}</span>
                </div>
              </li>
            ))}
          </ul>
